...
public class SequenceHuntGameModel implements Serializable
{
    ...
    /**
     * Value of a correct guess when calculating the relative score for a try.
     * This is used solely to rank the how close a try is to the previous try
     ...
     * Logger
     */
    private final static Logger LOG = Logger.getLogger(SequenceHuntGameModel.class);
    ...
    {
        log(SequenceHuntGameModel.class.getName(), "Requested sequence length: " + sequenceLength);
        setSequenceLength(pSequenceLength);
        log(SequenceHuntGameModel.class.getName(), "Resulting sequence length: " + getSequenceLength());
        setup();
    }
    ...
    {
        if (pSequenceLength >= MINIMUM_SEQUENCE_LENGTH && pSequenceLength <= MAXIMUM_SEQUENCE_LENGTH)
        {
            this.sequenceLength = pSequenceLength;
            log(SequenceHuntGameModel.class.getName(), "setSequenceLength, used supplied value: " + pSequenceLength);
        }
        else
        {
            this.sequenceLength = DEFAULT_SEQUENCE_LENGTH;
            log(SequenceHuntGameModel.class.getName(), "setSequenceLength, ignored supplied value: " + pSequenceLength
            + " and used default: " + this.sequenceLength);
        }
    }
    ...
    private void signalGameStart()
    {
        ...
        log(SequenceHuntGameModel.class.getName(), "signalGameStart at " + latestStartupDate.getTime());
    }
    ...
    {
        updateElapsedTime();
        log(SequenceHuntGameModel.class.getName(), "getElapsedTime returning " + elapsedMS);
        return elapsedMS;
    }
    ...
    public final void updateElapsedTime()
    {
        ...
        if (gameStarted && !isLoser() && !isWinner())
        {
            if (gameStarted && latestStartupDate != null)
            {
                ...
                log(SequenceHuntGameModel.class.getName(),
                "updateElapsedTime update start time [" + latestStartupDate.getTime() + "] date.getTime ["
            ...
            else
            {
                if (gameStarted)
                {
                    ...
                    latestStartupDate = new Date();
                    log(SequenceHuntGameModel.class.getName(),
                    "updateElapsedTime with no start time [" + latestStartupDate.getTime() + "]");
                }
            }
        }
    }
    ...
    {
        log(SequenceHuntGameModel.class.getName(), "signalGameEnd");
        updateElapsedTime();
        gameStarted = false;
    }
    ...
    {
        log(SequenceHuntGameModel.class.getName(), "signalGamePaused");
        updateElapsedTime();
    }
    ...
    public final void signalGameRestored()
    {
        latestStartupDate = new Date();
        log(SequenceHuntGameModel.class.getName(),
        "signalGameRestored latestStartupDate.getTime [" + latestStartupDate.getTime() + "]");
    }
    ...
    {
        log(SequenceHuntGameModel.class.getName(), "addGuess color [" + color + "]");
        if (!gameStarted)
        {
            signalGameStart();
        }
     ...
     */
    private void shuffleClues(final int numberOfCorrectPositionClues, final int numClues)
    {
        List<Integer> temp;
    ...
    {
        // log(SequenceHuntGameModel.class.getName(), "getSequenceLength
        // returning: " + sequenceLength);
    ...
    {
        return row < currentTry && clue[row][clueNum][CLUE_METADATA_TYPE] == UNSELECTED;
    }
    ...
    {
        log("Sequence", "Model getClueMeaning row: " + row + "  clueNum: " + clueNum);
     ...
     */
    public final Color getClueColorCode(final int row, final int clueNum)
    {
        Color color;
     ...
     */
    public final Color getTryColorCode(final int row, final int tryNum)
    {
        Color color;
